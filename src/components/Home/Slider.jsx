...
const Slider = ({ recommendation }) => {
    return (
        <div className="relative bg-white rounded-lg overflow-hidden shadow-lg group hover:shadow-2xl transition-shadow duration-300 w-full h-80">
            ...
            <div className="p-4">
                <div className="font-semibold text-lg text-heading">{recommendation?.recommendedBy}</div>
                <div className="text-paragraph">{recommendation?.tags?.join(', ')}</div>
            </div>
        </div>
    );
};
